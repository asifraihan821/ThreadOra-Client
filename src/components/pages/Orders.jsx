...
const Orders = () => {
    ...
    try {
      const response = await apiClientInterceptor.post(`/orders/${orderId}/cancel/`);
      if(response.status === 200){
        setOrders(prevOrder => prevOrder.map((order) => order.id === orderId ? {...order, status:'CANCELLED'} : order))
      }
    } catch (error) {
      console.log(error);
    }
