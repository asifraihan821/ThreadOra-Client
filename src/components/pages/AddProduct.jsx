...
const AddProduct = () => {
  ...
  const handleProductAdd = async (data) => {
    const slug = crypto.randomUUID(); // built-in JS function (Node 19+ / modern browsers)

    const payload = {
      ...data,
      slug, // add slug manually
    };
    try {
      const productRes = await apiClientInterceptor.post("/products/", payload);
  ...
  const handleUpload = async() => {
    ...
    try {
      ...
      await apiClientInterceptor.post(`/products/${productId}/images/`, formData);
      setLoading(false);
      }
