...
const OrderCard = ({ order, onCancel }) => {
  ...
  const handleStatusChange = async (event) => {
    ...
    try {
      const response = await apiClientInterceptor.patch(
  ...
  const handlePayment = async () => {
    setLoading(true);
    try {
      const response = await apiClientInterceptor.post("/payment/initiate/", {
        ...
        <div className="flex gap-2">
          {user.is_staff ? (
            <select
              ...
              className="px-3 py-1 rounded-full text-white text-sm font-md bg-blue-500"
            >
